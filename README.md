...
### Windows
Head over to the **Releases** tab, download the windows archive and extract it. If you're fine with using the ``watch`` mode (see the [Easy Usage](#easy-usage-example) and [Modes](#modes) sections above), simply double-click on the executable to launch the application. You can also create a **Shortcut** of the executable and pin it to your Start Menu or Taskbar for easier access.  
...
#### OCR
In order to use **OCR mode**, the OCR engine ``tesseract`` needs to be installed on your system and be in your **PATH**.  
